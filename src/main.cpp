...
DTMaterial **verPrestamosAntesDeFecha(std::string ci, DTFecha *fecha, int &cantPrestamos)
{
    ...
    for (int x = 0; x < lector->getCantidadPrestamos(); x++)
    {
        if (esPrestamoAnterioraFecha(lector->getPrestamos()[x]->getFechaPrestamo(), fecha))
        {
            ...
            DTMaterial *dtmaterial = new DTMaterial(material->getCodigo(), material->getTitulo(), material->getAnioPublicacion());
            resultado[indice] = dtmaterial;
            indice++;
        }
    }
    return resultado;
...
int main()
{
    ...
    case 5:
    {
        std::cout << "Ingresaste: Ver préstamos antes de fecha " << std::endl;
        std::cout << "Ingrese la CI: ";
        std::string ciLector;
        std::cin >> ciLector;
        std::cout << "Ingrese la fecha de registro (dd mm aaaa): ";
        ...
        DTFecha *fechaActual = new DTFecha(dia, mes, anio);
        int cantPrestamos;
        DTMaterial **resultado = verPrestamosAntesDeFecha(ciLector, fechaActual, cantPrestamos);
        for (int i = 0; i < cantPrestamos; i++)
        {
            std::cout << "Codigo: " << resultado[i]->codigo << std::endl;
            std::cout << "Titulo: " << resultado[i]->titulo << std::endl;
            std::cout << "Anio: " << resultado[i]->anioPublicacion << std::endl;
            std::cout << "------------------------" << std::endl;
            delete resultado[i];
        }
        delete fechaActual;
